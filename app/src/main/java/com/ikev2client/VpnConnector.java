...
public class VpnConnector {
    ...
    public static void connect(Context context, ProfileManager.Profile profile) {
        ...
        try {
            File certFile = saveCaCert(context, profile);
            File profileFile = createStrongSwanProfile(context, profile, certFile);
    ...
    private static File saveCaCert(Context context, ProfileManager.Profile profile) throws Exception {
        File cacheDir = context.getCacheDir();
        File certFile = new File(cacheDir, profile.name + "_ca.pem");
        FileWriter writer = new FileWriter(certFile);
        writer.write(profile.caCert);
        writer.close();
        return certFile;
    }
    
    private static File createStrongSwanProfile(Context context, ProfileManager.Profile profile, File certFile) throws Exception {
